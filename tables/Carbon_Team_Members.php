...
class Carbon_Team_Members extends Rest implements iRest
{
    
    public const TABLE_NAME = 'carbon_team_members';
    public const MEMBER_ID = 'carbon_team_members.member_id'; 
    public const TEAM_ID = 'carbon_team_members.team_id'; 
    public const USER_ID = 'carbon_team_members.user_id'; 
    public const ACCEPTED = 'carbon_team_members.accepted'; 

    public const PRIMARY = [
        'carbon_team_members.member_id',
    ];

    public const COLUMNS = [
        'carbon_team_members.member_id' => 'member_id','carbon_team_members.team_id' => 'team_id','carbon_team_members.user_id' => 'user_id','carbon_team_members.accepted' => 'accepted',
    ];

    public const PDO_VALIDATION = [
        'carbon_team_members.member_id' => ['binary', '2', '16'],'carbon_team_members.team_id' => ['binary', '2', '16'],'carbon_team_members.user_id' => ['binary', '2', '16'],'carbon_team_members.accepted' => ['tinyint', '0', '1'],
    ];
    ...
    {
        $inject = ':injection' . count(self::$injection) . 'carbon_team_members';
    ...
    */
    ...
    public static function Post(array $argv, string $dependantEntityId = null)
    {
        ...
        $sql = 'INSERT INTO StatsCoach.carbon_team_members (member_id, team_id, user_id, accepted) VALUES ( UNHEX(:member_id), UNHEX(:team_id), UNHEX(:user_id), :accepted)';
        ...
        $member_id = $id = $argv['carbon_team_members.member_id'] ?? self::beginTransaction('carbon_team_members', $dependantEntityId);
        $stmt->bindParam(':member_id',$member_id, 2, 16);
    
        $team_id = $argv['carbon_team_members.team_id'];
        $stmt->bindParam(':team_id',$team_id, 2, 16);
    
        $user_id = $argv['carbon_team_members.user_id'];
        $stmt->bindParam(':user_id',$user_id, 2, 16);
    
        $accepted =  $argv['carbon_team_members.accepted'] ?? '0';
    ...
    public static function validateSelectColumn($column) : bool {
        return (bool) preg_match('#(((((hex|argv|count|sum|min|max) *\(+ *)+)|(distinct|\*|\+|-|/| |carbon_team_members\.member_id|carbon_team_members\.team_id|carbon_team_members\.user_id|carbon_team_members\.accepted))+\)*)+ *(as [a-z]+)?#i', $column);
    }
    ...
    public static function buildSelectQuery(string $primary = null, array $argv, PDO $pdo = null, bool $noHEX = false) : string 
    {
        ...
        $sql = 'SELECT ' .  $sql . ' FROM StatsCoach.carbon_team_members ' . $join;
    ...
    */
    public static function Put(array &$return, string $primary, array $argv) : bool
    {
        ...
        $sql = 'UPDATE StatsCoach.carbon_team_members ';

        $sql .= ' SET ';        // my editor yells at me if I don't separate this from the above stmt

        $set = '';

        if (array_key_exists('carbon_team_members.member_id', $argv)) {
            $set .= 'member_id=UNHEX(:member_id),';
        }
        if (array_key_exists('carbon_team_members.team_id', $argv)) {
            $set .= 'team_id=UNHEX(:team_id),';
        }
        if (array_key_exists('carbon_team_members.user_id', $argv)) {
            $set .= 'user_id=UNHEX(:user_id),';
        }
        if (array_key_exists('carbon_team_members.accepted', $argv)) {
        ...
        if (array_key_exists('carbon_team_members.member_id', $argv)) {
            $member_id = $argv['carbon_team_members.member_id'];
            $stmt->bindParam(':member_id',$member_id, 2, 16);
        }
        if (array_key_exists('carbon_team_members.team_id', $argv)) {
            $team_id = $argv['carbon_team_members.team_id'];
        ...
        if (array_key_exists('carbon_team_members.accepted', $argv)) {
            $accepted = $argv['carbon_team_members.accepted'];
            $stmt->bindParam(':accepted',$accepted, 0, 1);
        }
        ...
        $argv = array_combine(
            array_map(
                static function($k) { return str_replace('carbon_team_members.', '', $k); },
                array_keys($argv)
            ),
            array_values($argv)
        );
    ...
    */
    public static function Delete(array &$remove, string $primary = null, array $argv) : bool
    {
        ...
        $sql = 'DELETE c FROM StatsCoach.carbons c 
                JOIN StatsCoach.carbon_team_members on c.entity_pk = carbon_team_members.member_id';
