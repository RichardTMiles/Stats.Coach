...
class Golf extends Request
{
    ...
    public function PostScore(&$state, &$course_id, &$boxColor)
    {
        ...
        if ($newScore) {
            ...
            $this->ffs = $this->post( 'ffs-1', 'ffs-2', 'ffs-3', 'ffs-4', 'ffs-5', 'ffs-6', 'ffs-7', 'ffs-8', 'ffs-9', 'ffs-10', 'ffs-11', 'ffs-12', 'ffs-13', 'ffs-14', 'ffs-15', 'ffs-16', 'ffs-17', 'ffs-18' )->int();
    ...
    public function AddCourse($state)
    {
        ...
        if ($state) $this->state = ucfirst( parent::set( $state )->alnum() );  // uri

        if (empty($_POST)) return false;

        $this->phone = $this->post( 'c_phone' )->phone();
        $this->pga_pro = $this->post( 'pga_pro' )->text();
        $this->course_website = $this->post( 'course_website' )->website();
        ...
        if (($this->tee_boxes = $this->post( 'tee_boxes' )->int(1,5)) === false)
        ...
        $this->par = $this->post( 'par_1', 'par_2', 'par_3', 'par_4', 'par_5', 'par_6', 'par_7', 'par_8', 'par_9', 'par_10', 'par_11', 'par_12', 'par_13', 'par_14', 'par_15', 'par_16', 'par_17', 'par_18' )->int();

        $validate( $this->par );

        for ($i = 1; $i <= $this->tee_boxes; $i++) {
            if (isset($_POST["tee_$i" . "_color"])) {
                $this->slope[$i] = $this->post( "general_slope_$i", "women_slope_$i" )->int();
                $this->difficulty[$i] = $this->post( "general_difficulty_$i", "women_difficulty_$i" )->int();
                $this->teeBox[$i] = $this->post( "tee_$i" . "_1", "tee_$i" . "_2", "tee_$i" . "_3", "tee_$i" . "_4", "tee_$i" . "_5", "tee_$i" . "_6", "tee_$i" . "_7", "tee_$i" . "_8", "tee_$i" . "_9", "tee_$i" . "_10", "tee_$i" . "_11", "tee_$i" . "_12", "tee_$i" . "_13", "tee_$i" . "_14", "tee_$i" . "_15", "tee_$i" . "_16", "tee_$i" . "_17", "tee_$i" . "_18" )->int();
        ...
        if (false === ($this->handicap_number = $this->post( 'Handicap_number' )->int(0,2)))
            throw new PublicAlert( 'Sorry, handicap number appears invalid' );

        switch ($this->handicap_number) {
            case 2:     // No break
                $this->handicap[2] = $this->post( "hc_2_1", "hc_2_2", "hc_2_3", "hc_2_4", "hc_2_5", "hc_2_6", "hc_2_7", "hc_2_8", "hc_2_9", "hc_2_10", "hc_2_11", "hc_2_12", "hc_2_13", "hc_2_14", "hc_2_15", "hc_2_16", "hc_2_17", "hc_2_18" )->int();
                $validate( $this->handicap[2] );
            case 1:
                $this->handicap[1] = $this->post( "hc_1_1", "hc_1_2", "hc_1_3", "hc_1_4", "hc_1_5", "hc_1_6", "hc_1_7", "hc_1_8", "hc_1_9", "hc_1_10", "hc_1_11", "hc_1_12", "hc_1_13", "hc_1_14", "hc_1_15", "hc_1_16", "hc_1_17", "hc_1_18" )->int();
